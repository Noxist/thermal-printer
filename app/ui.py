  ...
  <form method="post" action="/ui/print/template">
    <div class="grid">
      ...
      <div>
        ...
        <textarea id="lines" name="lines" placeholder="Lesen – 10 Min&#10;Wasser trinken"></textarea>
      </div>
    </div>
  ...
  <form method="post" action="/ui/print/image" enctype="multipart/form-data">
    <div class="grid">
      ...
      <div>
        ...
        <input id="img_title" type="text" name="img_title" placeholder="">
      </div>
      <div>
        <label for="img_subtitle">Untertitel (optional)</label>
        <input id="img_subtitle" type="text" name="img_subtitle" placeholder="">
      </div>
    </div>
    <div class="row" style="margin-top:12px">
...
def html_page_ui(auth_required: bool) -> HTMLResponse:
    html = HTML_UI.replace("{{AUTH_REQUIRED}}", "true" if auth_required else "false")
...
def _ui_handle_auth(request: Request, pass_: str | None, remember: bool):
    from .config import ui_auth_state
    authed, should_set_cookie = ui_auth_state(request, pass_, remember)
    return authed, should_set_cookie


@router.post("/ui/print/template", response_class=HTMLResponse)
async def ui_print_template(
    ...
    authed, set_cookie = _ui_handle_auth(request, pass_, remember)
    if not authed:
...
@router.post("/ui/print/raw", response_class=HTMLResponse)
...
@router.post("/ui/print/image", response_class=HTMLResponse)
